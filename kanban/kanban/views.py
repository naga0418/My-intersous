...
def index(request):
    return render(request,"kanban/index.html") #kanbanディレクトリの中のkanbanディレクトリの中にあるindex.htmlをHttpResponseに変換しているようだ

# @login_required
# def home(request):
#     return render(request,"kanban/home.html") #render(request, template_name(テンプレートの名前を記述))
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect("kanban:home") #アプリ名:ビュー名(urls/pyのname=''のところ)
    else:
...
class UserUpdateView(OnlyYouMixin,UpdateView):
    ...
    def get_success_url(self):
        return resolve_url('kanban:users_detail',pk=self.kwargs['pk']) #resolve_url()はビューの名称とPKの値をもとに/kanban/users/1/といったURLに変換する処理を行う
